...
LISS is a declarative, distro-neutral specification of a Linux installation: disks,
filesystems, boot, users, network, and software *intent*. A LISS document is produced
...
└──────┬──────┘   └──────┬──────┘   └──────┬───────┘      (produce LISS)
       └───────────────  ▼  ───────────────┘
                 ╔═══════════════╗
                 ║  system.liss  ║                        the standard
                 ╚═══════╤═══════╝
       ┌───────────────  ▼  ───────────────┐
┌──────┴──────┐   ┌──────┴──────┐   ┌──────┴───────┐
│ NixOS       │   │ Arch        │   │ Debian       │      appliers
│ applier     │   │ applier     │   │ applier      │      (consume LISS)
...
covers post-install configuration. Installation itself has no standard. LISS is that
standard.

## Design principles

1. **Declarative intent, not commands.** A LISS file states *what the system is*,
...
3. **No silent drift.** An applier MUST refuse a document containing core intent it
   cannot honor. A LISS file that applies successfully means the same thing everywhere.
4. **Machine-validated.** The canonical format is JSON with a published JSON Schema.
   ...
   pattern).
5. **Versioned.** Every document carries `"liss": "<semver>"`. Appliers declare which
...
- [`schema/liss-0.1.schema.json`](schema/liss-0.1.schema.json) — JSON Schema (draft 2020-12)
- [`examples/`](examples/) — complete documents, JSON and YAML
- [`docs/prior-art.md`](docs/prior-art.md) — what exists, what LISS learned from each

## Status

**v0.1.0-draft.** The spec is being developed alongside its first two implementations:

- **frontend**: the [nox](https://github.com/bresilla/nixos) installer TUI emits LISS
- **applier**: LISS → NixOS (disko.nix + configuration.nix generation)

An applier for Arch (LISS → archinstall configuration) is the planned second target,
...
- Image building (mkosi, Ignition). LISS describes an installation onto hardware.
