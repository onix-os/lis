...
def check_unsupported(doc: dict) -> None:
    ...
    if storage.get("encryption"):
        ...
        refuse(f"storage.encryption ({ids}): not yet implemented for Agama — the passphrase "
               "must reach the profile from seed key material at apply time "
               "(delivery.md §6), or the container be prepared before install")
    if network.get("wifi"):
