...
def render_alpine(doc: dict) -> tuple[str, str, str]:
    ...
    if not storage.get("wipe", False):
        ...
        warn("storage.wipe: false — the partition table is kept and the declared "
             "partitions are created in place; anything already on them is still "
             "replaced by the new filesystems")
    ...
    if len(disks) > 1 and not (storage.get("raid") or storage.get("lvm")):
        refuse(f"{len(disks)} disks declared but no storage.raid or storage.lvm "
               "spans them — setup-disk installs to a single device")
    disk = disks[0] if disks else "/dev/vda"

    # storage.lvm is built by lis-prepare.sh below: this applier partitions the
    # disk itself and only hands the mounted result to setup-disk, so a volume
    # group is as expressible as a plain partition.
    ...
    manual = bool((root and root.get("subvolumes"))
                  or storage.get("raid") or storage.get("lvm")
                  or storage.get("encryption"))
    ...
    for cmd in enrollment_commands(doc):
        post.append(f'chroot "$target" sh -c {json.dumps(cmd)}')
    for cmd in registration_commands(doc, "alpine"):
        post.append(f'chroot "$target" sh -c {json.dumps(cmd)}')
    for cmd in chroot_intents(doc, "alpine"):
        post.append(f'chroot "$target" sh -c {json.dumps(cmd)}')
...
def prepare_script(disk, partitions, root, boot_part, env, timeout=None,
    ...
    subvols = ((root or lvm_root) or {}).get("subvolumes", []) or []
    ...
    root_fs = role_fs(root) if root else (lvm_root or {}).get("fs")
    ...
    if loader == "grub":
        ...
        if crypt_over:
            # Name the container on the command line: Alpine's initramfs opens
            # cryptroot= into cryptdm= before it looks for the root filesystem.
            # Double-quoted, or the shell would pass the variable names through
            # literally and the initramfs would search for a device called
            # "$cryptuuid".
            params = '"' + params.strip("'") + ' cryptroot=UUID=$cryptuuid cryptdm=$cryptdm"' 
        steps += [
            'for d in dev proc sys; do mount --bind "/$d" "$mnt/$d"; done',
            'chroot "$mnt" apk add --no-progress grub '
            + ("grub-efi efibootmgr" if efi else "grub-bios"),
            # The initramfs can only open a LUKS device if it was built with the
            # cryptsetup feature; without this the install succeeds and the
            # system then cannot find its root.
            *(['chroot "$mnt" apk add --no-progress cryptsetup',
               'sed -i \'s/^features="\\(.*\\)"/features="\\1 cryptsetup"/\''
               ' "$mnt/etc/mkinitfs/mkinitfs.conf"',
               'chroot "$mnt" sh -c \'mkinitfs $(ls /lib/modules | head -n1)\'']
              if crypt_over else []),
