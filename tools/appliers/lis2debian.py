...
def render_storage(doc: dict, lines: list[str]) -> tuple[list[str], list[str]]:
    ...
    if storage.get("raid"):
        ...
        refuse(f"storage.raid ({names}): partman-md recipes are not generated by this "
    ...
    for disk in target.get("disks", []):
        ...
        disks[disk["id"]] = path
    if len(disks) > 1:
        refuse(f"{len(disks)} disks declared: partman-auto drives a single "
               "`partman-auto/disk`; multi-disk layouts need a partman-md/lvm recipe")
    if not disks:
        return late, late_last
