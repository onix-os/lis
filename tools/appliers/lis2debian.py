...
def render_storage(doc: dict, lines: list[str]) -> tuple[list[str], list[str]]:
    ...
    if storage.get("encryption"):
        ...
        refuse(f"storage.encryption ({ids}): partman-crypto needs "
               "partman-crypto/passphrase in the preseed, which SPEC §2.4 forbids the "
...
def render_preseed(doc: dict) -> str:
    ...
    lines.append(f"d-i keyboard-configuration/xkb-keymap select {keymap}")
