...
def make_conf(doc: dict, arch_dir: str, chost: str, packages: list[str]) -> str:
    ...
    lines = ["# Generated by lis2gentoo (Linux Installation Specification).",
             ...
             '# Note what is NOT here: --binpkg-respect-use=n. It looks like a pure',
             '# speed knob — take the published binary even when a USE flag differs',
             '# — but a package built with different USE is a different package, and',
             '# portage then resolves *its* dependencies. Asking for dev-vcs/git got',
             '# a binary built with gnome-keyring, which pulled gtk+, librsvg and a',
             '# 32-bit ABI across the whole tree and then failed. Matching USE',
             '# exactly means some packages build from source; that is the cost of',
             '# installing what the document actually asked for.',
...
def chroot_script(doc: dict, *, family: str, profile: str, disk: str,
    ...
    s = ["#!/bin/bash",
         ...
         "",
         "# Printed before anything can fail, so a run that produces nothing at all",
         "# says `arch-chroot` never got here rather than leaving it ambiguous.",
         'echo "lis2gentoo(chroot): entered the target"',
         "",
         "# -u is deliberately absent. A stage3's /etc/profile and /etc/env.d are",
         "# not written for `set -u`, and an unbound variable makes bash exit on the",
         "# spot with no way to catch it — which is exactly how this script first",
         "# failed: silently, before its first command, with the error swallowed.",
         "set -eo pipefail",
         "export PATH=/usr/sbin:/usr/bin:/sbin:/bin",
         "die() { echo \"lis2gentoo(chroot): $*\" >&2; exit 1; }",
         'step() { echo \"lis2gentoo(chroot): == $* ==\"; }',
         "",
         "step 'ebuild repository'",
         ...
         "",
         "step 'binary-package trust store'",
