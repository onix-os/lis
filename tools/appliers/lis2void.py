...
# Network managers Void packages and ships a runit service for. Anything else
# would leave chroot_intents pointing a runsvdir symlink at a service directory
# that does not exist, which runit ignores in silence.
VOID_NETWORK_MANAGERS = ("auto", "networkmanager", "connman")
...
def network_commands(doc: dict) -> list[str]:
    ...
    if manager is not None and manager not in VOID_NETWORK_MANAGERS:
        refuse(f"network.manager {manager!r}: Void ships no runit service for it "
               f"(supports {', '.join(VOID_NETWORK_MANAGERS)})")
...
def post_script(doc: dict, layout: Layout, params: list[str],
    ...
    if time_cfg:
        ...
        daemon = {"auto": "chronyd", "chrony": "chronyd",
                  "openntpd": "ntpd"}.get(provider)
        packaged = {"chronyd": "chrony", "ntpd": "openntpd"}.get(daemon)
        if time_cfg.get("ntp") is False:
            out.append("rm -f /etc/runit/runsvdir/default/chronyd "
                       "/etc/runit/runsvdir/default/ntpd")
        elif daemon is None:
            refuse(f"system.time.provider {provider!r} is not packaged for Void "
                   "(supports chrony, openntpd)")
        else:
            out.append(f"xbps-install -Sy {packaged}")
            for server in time_cfg.get("servers", []) or []:
                out.append(f"printf 'server %s iburst\\n' {q(server)} >> "
                           + ("/etc/chrony.conf" if daemon == "chronyd"
                              else "/etc/ntpd.conf"))
            out += service_link(daemon)
...
def render(doc: dict) -> str:
    ...
    if flatpaks := software.get("flatpak"):
        refuse(f"software.flatpak ({', '.join(map(str, flatpaks))}): this applier "
               "installs no flatpak runtime and adds no remote, so the app IDs "
               "would reach nothing")
    if layout.ok:
...
def main() -> int:
    ...
    check_section_fields(doc, "desktop", {"display_manager"})
    if (dm := (doc.get("desktop") or {}).get("display_manager")) == "ly":
        refuse("desktop.display_manager 'ly' is not in the Void repositories")
