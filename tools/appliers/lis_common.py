...
DRIVER_PACKAGES: dict[str, dict] = {
    ...
    # Void keeps Intel microcode and the NVIDIA drivers in void-repo-nonfree,
    # which base-system does not enable, so those names would resolve to nothing.
    # Checked against repo-default.voidlinux.org/current x86_64-repodata.
    "void": {
        "microcode": {"intel": None, "amd": "linux-firmware-amd"},
        "gpu": {"nvidia": None, "nvidia-open": None,
                "amdgpu": "mesa-dri", "intel": "mesa-dri"},
        "firmware": "linux-firmware",
    },
}
...
def driver_packages(doc: dict, family: str,
    ...
    if microcode not in (None, "auto", "none"):
        ...
        elif table["microcode"][microcode] is None:
            # Same trap as the GPU table above: appending None would put the
            # literal "None" in a package list and install nothing.
            refuse(f"drivers.microcode {microcode!r} is not in this distro's base "
                   "repositories — enable the vendor repository and name the "
                   "package in software.packages")
        else:
...
LOCALE_GEN = {
    ...
    # Void has no locale-gen: locales are uncommented in /etc/default/libc-locales
    # and regenerated by reconfiguring the glibc-locales package.
    "void": "xbps-reconfigure -f glibc-locales",
}

# Where a family lists the locales it wants generated, and how a line is spelled
# there. Void's file ships every locale commented out, so an entry has to be
# uncommented rather than appended — appending would leave a duplicate the
# generator ignores.
LOCALE_LIST_FILE = {"void": "/etc/default/libc-locales"}
...
# Void ships an apparmor package, but its kernel needs `security=apparmor
# apparmor=1` on the command line and a policy set before the LSM does anything;
# installing the package alone would look honored and enforce nothing.
LSM_PACKAGES["apparmor"]["void"] = None
...
def system_commands(doc: dict, family: str) -> list[str]:
    ...
    if extra := system.get("extra_locales"):
        ...
        elif listing := LOCALE_LIST_FILE.get(family):
            for loc in extra:
                charset = loc.split(".")[-1] if "." in loc else "UTF-8"
                out.append(f"sed -i {shlex.quote(f'/^#*{loc} /s/^#*//')} {listing}")
                out.append(f"grep -q {shlex.quote(f'^{loc} ')} {listing} "
                           f"|| echo {shlex.quote(f'{loc} {charset}')} >> {listing}")
            out.append(gen)
        else:
            for loc in extra:
                charset = loc.split(".")[-1] if "." in loc else "UTF-8"
...
PKG = {
    ...
    # runit has no "enable" command: a service is a symlink from /etc/sv into the
    # runsvdir, which names the service twice. Hence the {0} template — see
    # service_enable() below.
    "void": ("xbps-install -Sy", "xbps-remove -Ry",
             "ln -sfn /etc/sv/{0} /etc/runit/runsvdir/default/{0}"),
}


def service_enable(family: str, name: str) -> str:
    """The command that enables one service, however this family spells it.

    Six of the eight families take `<verb> <name>`; runit takes a symlink that
    needs the name twice, which a bare verb string cannot express. A verb
    containing a `{0}` placeholder is formatted instead of appended to.
    """
    verb = PKG[family][2]
    return verb.format(name) if "{0}" in verb else f"{verb} {name}"
...
def chroot_intents(doc: dict, family: str) -> list[str]:
    ...
    for snap in software.get("snap", []) or []:
        if family in ("alpine", "arch", "gentoo", "gentoo-systemd", "void"):
    ...
    if dm := desktop.get("display_manager"):
        if dm not in DISPLAY_MANAGERS and dm != "auto":
            ...
            out.append(f"{service_enable(family, DISPLAY_MANAGERS[dm])} || true")
    ...
    if manager not in (None, "auto"):
        ...
        if table is None:
            ...
            out.append(service_enable(
                family, "NetworkManager" if "NetworkManager" in pkg else pkg)
                + " || true")
        elif manager == "systemd-networkd":
            out.append(f"{service_enable(family, 'systemd-networkd')} || true")
    ...
    if firewall := network.get("firewall"):
        if family == "alpine":
            ...
            warn("network.firewall: no firewall is configured on Void by this "
                 "applier — iptables is packaged, but nothing here writes a "
                 "ruleset")
        else:
            svc = "firewalld" if family in ("fedora", "suse") else "ufw"
            out.append(f"{install} {svc}")
            out.append(f"{service_enable(family, svc)} || true")
    ...
    snapshots = (doc.get("storage", {}) or {}).get("snapshots") or {}
    if snapshots.get("enabled") and family not in ("alpine", "void"):
